...
class Bookings extends React.Component {
  ...
  getListing() {
    return axios.get(`http://localhost:3001/api/rooms/${this.state.listingId}/bookings/listingInfo`)
      .then((res) => {
        const { data } = res;
        this.setState({
          listingId: data.id,
  ...
  getCalendar() {
    ...
    axios.get(`http://localhost:3001/api/rooms/${this.state.listingId}/bookings/calendar/${yearMonthString}`)
      .then((res) => {
        const { data } = res;
        this.setState({
